...
class ObjectDetector:
    def __init__(self, model_path: str = DEFAULT_PATH, device: Optional[str] = None) -> None:
    ...
    def calculate_approach_vectors(self, roi: np.ndarray) -> List[Dict]:
        ...
        if np.any(mag_mask):
            hist, bins = np.histogram(direction[mag_mask], bins=8, range=(-np.pi, np.pi))
    ...
    def annotate_frame(self, frame: np.ndarray, boxes: List[Any], grasp_info: Optional[List[Dict]] = None) -> np.ndarray:
