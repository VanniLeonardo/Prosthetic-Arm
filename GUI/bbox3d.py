...
class BBox3DEstimator:
    ...
    def __init__(self, camera_matrix=None, projection_matrix=None, class_dims=None):
        ...
        # Frame dimensions for dynamic camera estimation
        self.frame_width = None
        self.frame_height = None

    def update_camera_params(self, camera_matrix=None, projection_matrix=None):
        """
    ...
    def set_frame_dimensions(self, width, height):
        """
        Set frame dimensions for dynamic camera parameter estimation
        ...
        # If no camera matrix provided, estimate it from frame dimensions
        if self.K is None:
            self.K = self._estimate_camera_matrix(width, height)

        # If no projection matrix provided, create a simple one
        if self.P is None:
            self.P = self._estimate_projection_matrix(self.K)
    ...
    def _estimate_camera_matrix(self, width, height):
        ...
        Returns:
            numpy.ndarray: Estimated camera intrinsic matrix
        """
        # Use typical FOV for webcams (60-90 degrees)
        # Focal length can be estimated as: f = (width/2) / tan(FOV/2)
        fov_degrees = 65
    ...
    def _estimate_projection_matrix(self, K):
        """
        Create a simple projection matrix from camera intrinsic matrix

        Args:
            K (numpy.ndarray): Camera intrinsic matrix

        Returns:
            numpy.ndarray: Estimated projection matrix
        """
        # Create a projection matrix with no rotation or translation
        P = np.zeros((3, 4))
        P[:3, :3] = K
        return P
    ...
    def _get_camera_matrix(self):
        """
        Get current camera matrix, estimating if necessary
        ...
        if self.frame_width is not None and self.frame_height is not None:
            return self._estimate_camera_matrix(self.frame_width, self.frame_height)
    ...
    def _get_projection_matrix(self):
        """
        Get current projection matrix, estimating if necessary
    ...
    def _backproject_point(self, x, y, depth):
        ...
        # Backproject to 3D - handle case where K may be singular
        try:
            point_3d = np.linalg.inv(K) @ point_2d * depth
        except np.linalg.LinAlgError:
            # Fallback to a simpler approach using normalized coordinates
            # Normalize based on image center and approximate focal length
            if self.frame_width is not None and self.frame_height is not None:
                cx, cy = self.frame_width / 2, self.frame_height / 2
                # Approximate focal length as frame width
                f = self.frame_width
            else:
                # Default values if frame dimensions unknown
                cx, cy = 320, 240
                f = 800
    ...
    def _estimate_orientation(self, bbox_2d, location, class_name):
        ...
        if 'person' in class_name.lower():
            ...
            if aspect_ratio > 1.5:
                ...
                # Use the position relative to the image center
                if self.frame_width is not None:
                    image_center_x = self.frame_width / 2
                else:
                    # Use 2D box center as reference
    ...
    def project_box_3d_to_2d(self, box_3d):
        ...
        corners_2d_homo = P @ corners_3d_homo
        corners_2d = corners_2d_homo[:2, :] / np.maximum(corners_2d_homo[2, :], 1e-5)
        ...
        if abs(mean_x - center_x) > (x2 - x1) or abs(mean_y - center_y) > (y2 - y1):
...
class XYView:
    ...
    def reset(self):
        ...
        for dist in [1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]:
            ...
            # Check if it's a whole number using a different approach
            is_whole_number = (dist == int(dist))
            thickness = 2 if is_whole_number else 1
            cv2.line(self.bev_image,
                     (self.origin_x - 5, y),
                     (self.origin_x + 5, y),
    ...
    def draw_box(self, box_3d, color=None):
        ...
        try:
            ...
            if 'bbox_2d' in box_3d:
                # Calculate position based on center and y-coordinate in the frame
                center_y_2d = (y1 + y2) / 2
                image_height = self.bev_image.shape[0]
                rel_y = (center_y_2d / image_height) + 0.65
                bev_y = -(self.origin_y - int(rel_y * self.height * 0.7))
            else:
                # Fallback to using only depth if no bbox information
            ...
            if 'bbox_2d' in box_3d:
                ...
                rel_x = (center_x_2d / image_width) - 1.0
                bev_x = self.origin_x + int(rel_x * self.width * 0.3)
            else:
                bev_x = self.origin_x
